...
async function loadPendingLeaveRequests() {
    ...
    try {
        ...
        console.log("Initial Leave Requests Data:", leaveRequests); // Debugging
        console.log("Initial Leave Requests Error:", leaveRequestsError); // Debugging
        ...
        // Penting: Pastikan filter Realtime cocok dengan filter kueri Anda
        // Supabase Realtime filter format: `kolom=eq.nilai` atau `kolom=in.(nilai1,nilai2)`
        const realtimeFilter = `employee_id=in.(${subordinateIds.join(',')})`;
        console.log("Realtime Filter for leave_requests:", realtimeFilter); // Debugging

        leaveRequestsSubscription = supabaseClient
            .channel('public:leave_requests') // Nama channel, bisa apa saja
            .on('postgres_changes', { 
                event: '*', // Dengarkan semua event (INSERT, UPDATE, DELETE)
                schema: 'public',
                table: 'leave_requests',
                filter: realtimeFilter // Gunakan filter yang dibangun
            }, (payload) => {
                ...
                loadAndDisplayLeaveRequestsFromDB(subordinateIds);
            })
            .subscribe();
...
async function loadAndDisplayLeaveRequestsFromDB(subordinateIds) {
    console.log("Realtime triggered: Reloading leave requests from DB..."); // Debugging
    try {
        const { data: leaveRequests, error: leaveRequestsError } = await supabaseClient
            .from('leave_requests')
        ...
        displayLeaveRequests(leaveRequests); // Panggil fungsi tampilan
    } catch (e) {
        console.error("Error reloading leave requests via realtime:", e);
...
async function updateLeaveRequestStatus(requestId, newStatus) {
    ...
    try {
        ...
        // setTimeout(() => { // Penundaan ini tidak lagi diperlukan dengan realtime
        //     try {
        //         location.reload(); // <--- BARIS INI DIKOMENTARI
        //     } catch (reloadError) {
